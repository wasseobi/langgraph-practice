...
def generate_reply(state: ChatState) -> Dict:
    ...
    if executed_result:
        if executed_result.get("success"):
            ...
            context_parts.append(f"작업 성공: {executed_result.get('action')}")
            context_parts.append(f"세부 정보: {executed_result.get('details', {})}")
        else:
            error_msg = executed_result.get("error", {}).get("message", "알 수 없는 오류")
            context_parts.append(f"작업 실패: {error_msg}")
    
    context = "\n".join(context_parts)
    
    system_prompt = """
    당신은 친절하고 전문적인 AI 어시스턴트입니다. 
    주어진 컨텍스트를 바탕으로 적절한 응답을 생성해주세요.
